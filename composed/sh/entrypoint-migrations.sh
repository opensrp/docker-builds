...
if [ ! -f /etc/migrations/.OpenMRS${APPLICATION_SUFFIX}/openmrs${APPLICATION_SUFFIX}-runtime.properties ]; then
	cat > /etc/migrations/.OpenMRS${APPLICATION_SUFFIX}/openmrs${APPLICATION_SUFFIX}-runtime.properties <<- EOF
		...
		connection.url=jdbc:mysql://${MYSQL_HOST}:3306/${MYSQL_OPENMRS_DATABASE}?autoReconnect=true&sessionVariables=default_storage_engine=InnoDB&useUnicode=true&characterEncoding=UTF-8&zeroDateTimeBehavior=convertToNull
